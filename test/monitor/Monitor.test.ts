...

      {
        for (const txid of expectedTxids) {
          ...
          const req = verifyTruthy(
            await entity.EntityProvenTxReq.fromStorageTxid(storage, txid)
          )
    ...
    for (const { activeStorage: storage, monitor } of ctxs) {
...

      {
        ...
        for (const txid of expectedTxids) {
          ...
          const notifyIds =
            new entity.EntityProvenTxReq(req).notify.transactionIds || []
        ...
        for (const txid of expectedTxids) {
          ...
          if (env.logTests)
            console.log(new entity.EntityProvenTxReq(req).historyPretty())
          const notifyIds =
            new entity.EntityProvenTxReq(req).notify.transactionIds || []
