...
describe('CertificateLifeCycle tests', () => {
  ...
  test('2a complete flow MasterCertificate and VerifiableCertificate', async () => {
    ...
    const r1 = await MasterCertificate.createCertificateFields(certifierWallet, subject.toPublicKey().toString(), cert.fields)
    ...
    const r2 = await MasterCertificate.decryptFields(subjectWallet, r1.masterKeyring, signedCert.fields, signedCert.certifier)
    ...
    const r3 = await MasterCertificate.createKeyringForVerifier(subjectWallet, certifier.toPublicKey().toString(), verifier.toPublicKey().toString(), signedCert.fields, ['name', 'email'], r1.masterKeyring, signedCert.serialNumber)
...
function makeSampleCert(subjectRootKeyHex?: string, certifierKeyHex?: string, verifierKeyHex?: string): {
